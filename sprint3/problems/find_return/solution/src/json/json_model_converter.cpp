...
LootGeneratorConfig tag_invoke(json::value_to_tag<LootGeneratorConfig>,
                               const json::value &jv) {
  LootGeneratorConfig config;
  config.period =
      json::value_to<double>(jv.as_object().at(LOOT_GENERATOR_PERIOD));
  config.probability =
...
void tag_invoke(json::value_from_tag, json::value &jv,
  ...
  json::object res = {{LOOT_TYPES_NAME, json::value_from(loot_type.name)},
...
LootType tag_invoke(json::value_to_tag<LootType>, const json::value &jv) {
  LootType loot_type;
  loot_type.name =
      json::value_to<std::string>(jv.as_object().at(LOOT_TYPES_NAME));
  loot_type.file =
      json::value_to<std::string>(jv.as_object().at(LOOT_TYPES_FILE));
  loot_type.type =
      json::value_to<std::string>(jv.as_object().at(LOOT_TYPES_TYPE));
  loot_type.value = json::value_to<size_t>(jv.as_object().at(LOOT_TYPES_VALUE));
  try {
    loot_type.rotation =
        json::value_to<int>(jv.as_object().at(LOOT_TYPES_ROTATION));
  } catch (boost::wrapexcept<std::out_of_range> e) {
  };
  try {
    loot_type.color =
        json::value_to<std::string>(jv.as_object().at(LOOT_TYPES_COLOR));
  } catch (boost::wrapexcept<std::out_of_range> e) {
  };
...
Map tag_invoke(json::value_to_tag<Map>, const json::value &jv) {
  ...
  std::vector<Road> roads =
      json::value_to<std::vector<Road>>(jv.as_object().at(ROADS));
  map.AddRoads(roads);
  std::vector<Building> buildings =
      json::value_to<std::vector<Building>>(jv.as_object().at(BUILDINGS));
  map.AddBuildings(buildings);
  std::vector<Office> offices =
      json::value_to<std::vector<Office>>(jv.as_object().at(OFFICES));
  map.AddOffices(offices);
  std::vector<LootType> look_types =
      json::value_to<std::vector<LootType>>(jv.as_object().at(LOOT_TYPES));
  if (look_types.empty()) {
  ...
  try {
    double dog_velocity =
        json::value_to<double>(jv.as_object().at(MAP_DOG_VELOCITY));
    map.SetDogVelocity(dog_velocity);
  } catch (boost::wrapexcept<std::out_of_range> e) {
  }
  try {
    double bag_capacity =
        json::value_to<double>(jv.as_object().at(MAP_BAG_CAPACITY));
...
void tag_invoke(json::value_from_tag, json::value &jv,
                const LostObject &lost_object) {
  json::array pos;
  pos.push_back(lost_object.GetX());
  pos.push_back(lost_object.GetY());

  jv = {{"type", lost_object.GetType()}, {"pos", std::move(pos)}};
}
