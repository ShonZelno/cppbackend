...
std::optional<size_t> TimeTickHandler(
    ...
    if(!application.IsManualTimeManagement()) {
        std::cout << "    Manual time management is disabled" << std::endl;
        return 0;
    }
    
    auto delta_time_opt = json_converter::ParseSetDeltaTimeRequest(req.body());
    if (!delta_time_opt) {
        std::cout << "    Failed to parse delta_time" << std::endl;
        return 0;
    }
    
    int delta_time = delta_time_opt.value();
    
    net::dispatch(*application.GetStrand(), [req = std::move(req), application = &application, send = std::move(send), delta_time]{
...
template <typename Request>
bool MapInvalidMethodActivator(const Request& req) {
    auto url = SplitUrl(req.target());
    return url.size() == SIZE_OF_FOUR_SEGMENT_URL &&
           url[0] == "api" &&
           url[1] == "v1" &&
...
template <typename Request, typename Send>
std::optional<size_t> MapInvalidMethodHandler(
        const Request& req,
        app::Application& application,
        Send&& send) {
    StringResponse response(http::status::method_not_allowed, req.version());
