...
std::string CreateGameStateResponse(const std::vector< std::weak_ptr<app::Player> >& players) {
    ...
    for(auto item : players) {
        ...
        json::array speed;
        speed.push_back(dog->GetVelocity().vx);
        speed.push_back(dog->GetVelocity().vy);
        
        players_obj[ss.str()] = {
            {json_keys::RESPONSE_DOG_POSITION, pos},
            {json_keys::RESPONSE_DOG_VELOCITY, speed},
            {json_keys::RESPONSE_DOG_DIRECTION, model::DIRECTION_TO_STRING.at(dog->GetDirection())}
        };
    }
 ...
 json::object lost_objects_obj;
    if (!players.empty()) {
        auto first_player = players.front().lock();
        if (first_player) {
            const auto& game_session = first_player->GetGameSession();
             auto map = game_session->GetMap(); 
            size_t id = 0;
            for (const auto& obj : map->GetLostObjects()) {  // получаем lost objects из map
                json::array pos;
                pos.push_back(obj.GetX());
                pos.push_back(obj.GetY());
                
                json::object lost_object_data;
                lost_object_data["type"] = obj.GetType();
                lost_object_data["pos"] = pos;
                
                lost_objects_obj[std::to_string(id++)] = lost_object_data;
            }
        }
    }
    result["lostObjects"] = lost_objects_obj;
