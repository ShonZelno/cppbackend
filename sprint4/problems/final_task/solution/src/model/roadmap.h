...
public:
  ...
  GetValidMove(const geom::Point2D &old_position,
...
private:
  ...
  using MatrixMap =
      std::map<int64_t, std::map<int64_t, std::unordered_set<size_t>>>;
  MatrixMap matrix_map_;
  Roads roads_;

  std::optional<const MatrixMapCoord>
  GetDestinationRoadsOfRoute(std::optional<const MatrixMapCoord> start,
                             std::optional<const MatrixMapCoord> end,
                             const Velocity &old_velocity);
  std::optional<const MatrixMapCoord>
  GetCoordinatesOfPosition(const geom::Point2D &position);
  const Direction VelocityToDirection(const Velocity &velocity);
  const std::unordered_map<Direction, geom::Point2D>
  MatrixCoordinateToPosition(const MatrixMapCoord &coord,
