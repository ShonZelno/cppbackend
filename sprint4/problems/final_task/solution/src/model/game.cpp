...
void Game::AddMap(const Map &map) {
  ...
  if (auto [it, inserted] = map_id_to_index_.emplace(map.GetId(), index);
    ...
    try {
      ...
      if (default_dog_velocity_ && std::abs(current_map->GetDogVelocity() -
                                            INITIAL_DOG_VELOCITY) < EPSILON) {
...
double Game::GetDefaultDogRetirementTime() const noexcept {
  return default_dog_retirement_time_.value_or(DEFAULT_RETIREMENT_TIME);
}
