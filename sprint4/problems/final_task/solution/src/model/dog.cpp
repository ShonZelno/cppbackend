...
void Dog::UpdateIdleTime(std::chrono::milliseconds delta, bool is_moving) {
  if (is_moving) {
    ...
    idle_time_accumulated_ += delta;
  }
}

bool Dog::IsRetired(std::chrono::milliseconds retirement_time) const {
  return idle_time_accumulated_ >= retirement_time;
}

void Dog::ResetIdleTime() {
  idle_time_accumulated_ = std::chrono::milliseconds::zero();
}
