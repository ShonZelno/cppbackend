...
const std::unordered_set<std::string_view> GAME_API_URLS_WITH_AUTHORIZATION = {
    api_urls::GET_PLAYERS_LIST_API, api_urls::GET_PLAYERS_LIST_API + "/",
    api_urls::GET_GAME_STATE_API,   api_urls::GET_GAME_STATE_API + "/",
    api_urls::MAKE_ACTION_API,      api_urls::MAKE_ACTION_API + "/"};

const std::unordered_set<std::string_view> GAME_API_URLS_WITH_JSON_REQ = {
    api_urls::JOIN_TO_GAME_API, api_urls::JOIN_TO_GAME_API + "/",
    api_urls::MAKE_ACTION_API, api_urls::MAKE_ACTION_API + "/"};

const std::string CONTENT_TYPE_APPLICATION_JSON = "application/json";
const std::string NO_CACHE_CONTROL = "no-cache";

template <typename Request> bool BadRequestActivator(const Request &req) {
  ...
  return !url.empty() && url[0] == "api" &&
         (url.size() > SIZE_OF_FIVE_SEGMENT_URL ||
          ...
          (url.size() >= SIZE_OF_TWO_SEGMENT_URL && url[1] != "v1") ||
...
std::optional<size_t>
BadRequestHandler(const Request &req,
...
template <typename Request, typename Send>
std::optional<size_t>
GetMapListHandler(const Request &req,
                  std::shared_ptr<app::Application> application, Send &&send) {
  StringResponse response(http::status::ok, req.version());
  response.set(http::field::content_type, CONTENT_TYPE_APPLICATION_JSON);
  response.set(http::field::cache_control, NO_CACHE_CONTROL);
  response.body() =
      json_converter::ConvertMapListToJson(application->ListMap());
  response.content_length(response.body().size());
  response.keep_alive(req.keep_alive());
  send(response);
  return std::nullopt;
...
template <typename Request> bool GetMapByIdActivator(const Request &req) {
  auto url = SplitUrl(req.target());
  return url.size() == SIZE_OF_FOUR_SEGMENT_URL && url[0] == "api" &&
         url[1] == "v1" && url[2] == "maps";
}

template <typename Request, typename Send>
std::optional<size_t>
GetMapByIdHandler(const Request &req,
                  std::shared_ptr<app::Application> application, Send &&send) {
...
template <typename Request, typename Send>
std::optional<size_t>
MapNotFoundHandler(const Request &req,
                   std::shared_ptr<app::Application> application, Send &&send) {
...
template <typename Request, typename Send>
std::optional<size_t>
JoinToGameInvalidJsonReqHandler(const Request &req,
...
template <typename Request, typename Send>
std::optional<size_t>
JoinToGameEmptyPlayerNameHandler(const Request &req,
                                 std::shared_ptr<app::Application> application,
                                 Send &&send) {
  StringResponse response(http::status::bad_request, req.version());
  response.set(http::field::content_type, CONTENT_TYPE_APPLICATION_JSON);
...
template <typename Request> bool JoinToGameActivator(const Request &req) {
  return IsEqualUrls(api_urls::JOIN_TO_GAME_API, req.target());
}

template <typename Request, typename Send>
std::optional<size_t>
JoinToGameHandler(const Request &req,
                  std::shared_ptr<app::Application> application, Send &&send) {
  auto [player_name, map_id] =
      json_converter::ParseJoinToGameRequest(req.body()).value();
  ...
  if (session) {
    ...
    net::dispatch(*(session->GetStrand()), [&res_promise, application,
                                            &player_name, &map_id] {
      auto [token, player_id] = application->JoinGame(player_name, map_id);
      res_promise.set_value(
          json_converter::CreateJoinToGameResponse(*token, *player_id));
    });
...
template <typename Request, typename Send>
std::optional<size_t>
JoinToGameMapNotFoundHandler(const Request &req,
                             std::shared_ptr<app::Application> application,
                             Send &&send) {
  StringResponse response(http::status::not_found, req.version());
  response.set(http::field::content_type, CONTENT_TYPE_APPLICATION_JSON);
...
template <typename Request, typename Send>
std::optional<size_t>
OnlyPostMethodAllowedHandler(const Request &req,
                             std::shared_ptr<app::Application> application,
                             Send &&send) {
...
template <typename Request, typename Send>
std::optional<size_t>
EmptyAuthorizationHandler(const Request &req,
                          std::shared_ptr<app::Application> application,
                          Send &&send) {
...
template <typename Request> bool GetPlayersListActivator(const Request &req) {
  return IsEqualUrls(api_urls::GET_PLAYERS_LIST_API, req.target());
}

template <typename Request, typename Send>
std::optional<size_t>
GetPlayersListHandler(const Request &req,
                      std::shared_ptr<app::Application> application,
                      Send &&send) {
  ...
  net::dispatch(*(application->FindGameSessionBy(token)->GetStrand()),
                [&res_promise, &token, application] {
                  auto players = application->GetPlayersFromGameSession(token);
                  res_promise.set_value(
                      json_converter::CreatePlayersListOnMapResponse(players));
                });
...
template <typename Request, typename Send>
std::optional<size_t>
InvalidMethodHandler(const Request &req,
                     std::shared_ptr<app::Application> application,
                     Send &&send) {
  StringResponse response(http::status::method_not_allowed, req.version());
  response.set(http::field::content_type, CONTENT_TYPE_APPLICATION_JSON);
...
template <typename Request, typename Send>
std::optional<size_t>
GetGameStateHandler(const Request &req,
                    std::shared_ptr<app::Application> application,
                    Send &&send) {
  authentication::Token token{GetTokenString(req[http::field::authorization])};
  if (!application->IsExistPlayer(token)) {
  ...
  auto res_future = res_promise.get_future();
  net::dispatch(
      *(application->FindGameSessionBy(token)->GetStrand()),
      [&res_promise, &token, application] {
        auto players = application->GetPlayersFromGameSession(token);
        res_promise.set_value(json_converter::CreateGameStateResponse(
            players, application->FindGameSessionBy(token)->GetLostObjects()));
      });
  auto res = res_future.get();
  if (std::holds_alternative<size_t>(res)) {
    return std::get<size_t>(res);
...
template <typename Request, typename Send>
std::optional<size_t>
InvalidContentTypeHandler(const Request &req,
                          std::shared_ptr<app::Application> application,
                          Send &&send) {
  StringResponse response(http::status::bad_request, req.version());
  response.set(http::field::content_type, CONTENT_TYPE_APPLICATION_JSON);
...
template <typename Request, typename Send>
std::optional<size_t>
PlayerActionInvalidActionHandler(const Request &req,
                                 std::shared_ptr<app::Application> application,
                                 Send &&send) {
  StringResponse response(http::status::bad_request, req.version());
  response.set(http::field::content_type, CONTENT_TYPE_APPLICATION_JSON);
...
template <typename Request> bool PlayerActionActivator(const Request &req) {
  return IsEqualUrls(api_urls::MAKE_ACTION_API, req.target());
}

template <typename Request, typename Send>
std::optional<size_t>
PlayerActionHandler(const Request &req,
                    std::shared_ptr<app::Application> application,
                    Send &&send) {
  authentication::Token token{GetTokenString(req[http::field::authorization])};
  if (!application->IsExistPlayer(token)) {
  ...
  net::dispatch(
      *(application->FindGameSessionBy(token)->GetStrand()),
      [&res_promise, &token, &req, application] {
        std::string directionStr =
            json_converter::ParsePlayerActionRequest(req.body()).value();
        application->SetPlayerAction(
            token, model::STRING_TO_DIRECTION.at(directionStr));
...
template <typename Request, typename Send>
std::optional<size_t>
UnknownTokenHandler(const Request &req,
                    std::shared_ptr<app::Application> application,
                    Send &&send) {
  StringResponse response(http::status::unauthorized, req.version());
  response.set(http::field::content_type, CONTENT_TYPE_APPLICATION_JSON);
...
template <typename Request, typename Send>
std::optional<size_t>
PageNotFoundHandler(const Request &req,
                    std::shared_ptr<app::Application> application,
                    Send &&send) {
  StringResponse response(http::status::not_found, req.version());
  response.set(http::field::content_type, CONTENT_TYPE_APPLICATION_JSON);
...
template <typename Request, typename Send>
std::optional<size_t>
TimeTickInvalidMsgHandler(const Request &req,
                          std::shared_ptr<app::Application> application,
                          Send &&send) {
...
template <typename Request> bool TimeTickActivator(const Request &req) {
  return IsEqualUrls(api_urls::MAKE_TIME_TICK_API, req.target());
}

template <typename Request, typename Send>
std::optional<size_t>
TimeTickHandler(const Request &req,
                std::shared_ptr<app::Application> application, Send &&send) {
  if (!application->IsManualTimeManagement()) {
    return 0;
  }
  std::chrono::milliseconds dtime(
      json_converter::ParseSetDeltaTimeRequest(req.body()).value());
...
template <typename Request, typename Send>
std::optional<size_t>
InvalidEndpointHandler(const Request &req,
                       std::shared_ptr<app::Application> application,
                       Send &&send) {
  StringResponse response(http::status::bad_request, req.version());
  response.set(http::field::content_type, CONTENT_TYPE_APPLICATION_JSON);
...
template <typename Request> bool GetRecordsActivator(const Request &req) {
  return req.target().starts_with(api_urls::GET_RECORDS_API);
}
...
GetRecordsHandler(const Request &req,
  ...
  if (query_pos != std::string::npos) {
    ...
    for (const auto &param : params) {
      ...
      } else if (param.substr(0, 9) == "maxItems=") {
        try {
  ...
  auto records = application->GetRecords(start, max_items);
  std::string body = json_converter::CreateRecordsResponse(records);
