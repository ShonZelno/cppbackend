...
std::shared_ptr<GameSession> Player::GetGameSession() { return session_; }

void Player::SetGameSession(std::shared_ptr<GameSession> session) {
  session_ = session;
}

std::weak_ptr<model::Dog> Player::GetDog() { return dog_; }

void Player::SetDog(std::weak_ptr<model::Dog> dog) { dog_ = dog; }
