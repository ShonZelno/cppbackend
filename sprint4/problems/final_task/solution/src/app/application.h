...
public:
  std::vector<RetiredRecord> GetRecords(size_t start, size_t limit);

  using AppStrand = net::strand<net::io_context::executor_type>;

  Application(model::Game game, size_t tick_period, bool randomize_spawn_points,
              net::io_context &ioc)
      : game_{std::move(game)}, tick_period_{tick_period},
        randomize_spawn_points_{randomize_spawn_points}, ioc_{ioc} {};
  ...
  FindMap(const model::Map::Id &id) const noexcept;
  ...
  FindGameSessionBy(const model::Map::Id &id) const noexcept;
  std::shared_ptr<GameSession>
  FindGameSessionBy(const authentication::Token &token) const noexcept;
...
private:
  ...
  using GameSessionIdToIndex =
      std::unordered_map<GameSession::Id, std::vector<std::shared_ptr<Player>>,
                         GameSessionIdHasher>;
  using MapIdHasher = util::TaggedHasher<model::Map::Id>;
  using MapIdToSessionIndex =
      std::unordered_map<model::Map::Id, size_t, MapIdHasher>;
  using AuthTokenToSessionIndex =
      std::unordered_map<authentication::Token, std::shared_ptr<GameSession>,
                         authentication::TokenHasher>;
  using TokenToPlayer =
      std::unordered_map<authentication::Token, std::shared_ptr<app::Player>,
                         authentication::TokenHasher>;
  using GameSessionToTokenPlayerPair =
      std::unordered_map<std::shared_ptr<GameSession>, TokenToPlayer>;
  ...
  std::chrono::milliseconds total_game_time_{0};
  std::unordered_map<model::Dog::Id, std::shared_ptr<Player>,
