...
void GameSession::Run() {
  if (period_of_update_game_state_.count() != 0) {
    update_game_state_ticker_ = std::make_shared<time_m::Ticker>(
        strand_, period_of_update_game_state_,
  ...
  generate_loot_ticker_ = std::make_shared<time_m::Ticker>(
      strand_, loot_generator_.GetPeriod(),
      [self = shared_from_this()](const TimeInterval &delta_time) {
...
void GameSession::UpdateGameState(const GameSession::TimeInterval &delta_time) {
  for (auto &[dog_id, dog] : dogs_) {
    bool had_velocity =
        !(dog->GetVelocity().vx == 0 && dog->GetVelocity().vy == 0);
    auto [new_position, new_velocity] = map_->GetValidMove(
        dog->GetPosition(), dog->CalculateNewPosition(delta_time),
        dog->GetVelocity());
    dog->SetPosition(new_position);
    dog->SetVelocity(new_velocity);
    dog->UpdateIdleTime(delta_time, had_velocity);
  }
  HandleLoot();

  std::vector<model::Dog::Id> retired_dogs;
  for (auto &[id, dog] : dogs_) {
    if (dog->IsRetired(retirement_time_)) {
      retired_dogs.push_back(id);
    }
  }

  for (auto id : retired_dogs) {
    if (on_dog_retired_) {
      on_dog_retired_(id);
    }
    dogs_.erase(id);
...
void GameSession::GenerateLoot(const GameSession::TimeInterval &delta_time) {
  auto num_new_lost_obj =
      loot_generator_.Generate(delta_time, lost_objects_.size(), dogs_.size());
...
void GameSession::CollectLoot(const model::ItemDogProvider &provider,
  ...
  const auto *const casted_lost_obj =
      provider.TryCastItemTo<model::LostObject>(item_id);
  if (casted_lost_obj) {
...
void GameSession::DropLoot(const model::ItemDogProvider &provider,
                           size_t item_id, size_t gatherer_id) {
  auto dog = dogs_[provider.GetDogId(gatherer_id)];
  const auto *const casted_office =
      provider.TryCastItemTo<model::Office>(item_id);
  if (casted_office) {
    if (dog->IsEmptyBag()) {
      return;
    }
