...
public:
  ...
  GameSessionSerialization(
      ...
      const std::unordered_map<authentication::Token,
                               std::shared_ptr<app::Player>,
                               authentication::TokenHasher> &tokenToPalyer)
      : map_id_(*(game_session.GetMap()->GetId())) {
    std::ranges::transform(
        tokenToPalyer, std::back_inserter(players_ser_),
        [](const auto &token_to_player) -> PlayerSerialization {
          return PlayerSerialization(*token_to_player.second,
                                     token_to_player.first);
        });
    std::ranges::transform(
        game_session.GetLostObjects(), std::back_inserter(lost_objects_),
  ...
  GetLostObjectsSerialize() const;
  [[nodiscard]] const std::vector<PlayerSerialization> &
  GetPlayersSerialize() const;
