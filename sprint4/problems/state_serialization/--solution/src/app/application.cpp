...
namespace app
{
    ...
    void Application::RestoreGameState(saving::SavingSettings saving_settings)
    {
        ...
        RestoreGame();
        if (!(saving_settings_.state_file_path && saving_settings_.period) || IsManualTimeManagement())
        {
            return;
        }
    ...
    void Application::SaveGameState(const std::chrono::milliseconds &delta_time)
    {
        static int period = saving_settings_.period
                                ? saving_settings_.period.value().count()
                                : 0;
    ...
    void Application::RestoreGame()
    {
        ...
        {
            auto game_session = std::make_shared<GameSession>(game_.FindMap(item.RestoreMapId()), tick_period_, game_.GetLootGeneratorConfig(), ioc_);
