...
namespace app
{
    ...
    public:
        ...
        Application(model::Game game, size_t tick_period, bool randomize_spawn_points, net::io_context &ioc) : game_{std::move(game)},
                                                                                                               tick_period_{std::chrono::milliseconds(tick_period)}, // конвертируем size_t в milliseconds
                                                                                                               randomize_spawn_points_{randomize_spawn_points},
                                                                                                               ioc_{ioc},
                                                                                                               strand_{std::make_shared<AppStrand>(net::make_strand(ioc))}
        {
            if (tick_period != 0)
            { // проверяем исходное значение, а не tick_period_.count()
                ticker_ = std::make_shared<time_m::Ticker>(
                    strand_,
                    tick_period_, // здесь уже milliseconds
