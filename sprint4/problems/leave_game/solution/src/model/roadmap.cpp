...
Roadmap::Roadmap(const Roadmap &other) { CopyContent(other.roads_); };
...
void Roadmap::AddRoad(const Road &road) {
  const auto SCALLED_OFFSET =
      static_cast<int64_t>(OFFSET * SCALE_FACTOR_OF_CELL);
  auto index = roads_.size();
  roads_.emplace_back(road);
  if (road.IsHorizontal()) {
    auto start = static_cast<int64_t>((road.GetStart().x < road.GetEnd().x)
                                          ? road.GetStart().x
                                          : road.GetEnd().x);
    auto end = static_cast<int64_t>((road.GetStart().x < road.GetEnd().x)
                                        ? road.GetEnd().x
                                        : road.GetStart().x);
    ...
    auto start = static_cast<int64_t>((road.GetStart().y < road.GetEnd().y)
                                          ? road.GetStart().y
                                          : road.GetEnd().y);
    auto end = static_cast<int64_t>((road.GetStart().y < road.GetEnd().y)
                                        ? road.GetEnd().y
                                        : road.GetStart().y);
    start = start * SCALE_FACTOR_OF_CELL - SCALLED_OFFSET;
    end = end * SCALE_FACTOR_OF_CELL + SCALLED_OFFSET;
...
Roadmap::GetValidMove(const geom::Point2D &old_position,
  ...
  if (dest && IsValidPosition(matrix_map_[dest.value().x][dest.value().y],
                              potential_new_position)) {
...
geom::Point2D Roadmap::GenerateValidRandomPosition() const {
  ...
  if (road.IsHorizontal()) {
    pos.x =
        utils::GenerateDoubleFromInterval(road.GetStart().x, road.GetEnd().x);
    pos.y = road.GetStart().y;
  } else {
    pos.y =
        utils::GenerateDoubleFromInterval(road.GetStart().y, road.GetEnd().y);
...
Roadmap::GetDestinationRoadsOfRoute(std::optional<const MatrixMapCoord> start,
  ...
  if (old_velocity.vx != 0) {
    ...
    if (end) {
      ...
      end_x =
          (direction > 0) ? LLONG_MAX : -(OFFSET * SCALE_FACTOR_OF_CELL) - 1;
    }
    int64_t index{0};
...
Roadmap::GetCoordinatesOfPosition(const geom::Point2D &position) {
  ...
  int64_t x_index = (position.x >= 0)
                        ? std::floor(position.x * SCALE_FACTOR_OF_CELL)
                        : std::ceil(position.x * SCALE_FACTOR_OF_CELL);
  int64_t y_index = (position.y >= 0)
                        ? std::floor(position.y * SCALE_FACTOR_OF_CELL)
                        : std::ceil(position.y * SCALE_FACTOR_OF_CELL);
...
Roadmap::GetFarestPoinOfRoute(const MatrixMapCoord &roads_coord,
  ...
  for (auto road_ind : matrix_map_[roads_coord.x][roads_coord.y]) {
    auto start_position =
        cell_pos.at(DIRECTION_TO_OPOSITE_DIRECTION.at(direction));
...
Roadmap::MatrixCoordinateToPosition(const MatrixMapCoord &coord,
  ...
  res[Direction::NORTH] = geom::Point2D{
      target_position.x, (static_cast<double>(coord.y + y_inc_n) /
                          static_cast<double>(SCALE_FACTOR_OF_CELL))};
  res[Direction::SOUTH] = geom::Point2D{
      target_position.x, (static_cast<double>(coord.y + y_inc_s) /
                          static_cast<double>(SCALE_FACTOR_OF_CELL))};
  res[Direction::WEST] =
      geom::Point2D{(static_cast<double>(coord.x + x_inc_w) /
                     static_cast<double>(SCALE_FACTOR_OF_CELL)),
                    target_position.y};
  res[Direction::EAST] =
      geom::Point2D{(static_cast<double>(coord.x + x_inc_e) /
                     static_cast<double>(SCALE_FACTOR_OF_CELL)),
                    target_position.y};
...
bool Roadmap::IsValidPositionOnRoad(const Road &road,
  ...
  if (road.IsHorizontal()) {
    start_x = (road.GetStart().x < road.GetEnd().x) ? (road.GetStart().x)
                                                    : (road.GetEnd().x);
    end_x = (road.GetStart().x < road.GetEnd().x) ? (road.GetEnd().x)
                                                  : (road.GetStart().x);
    ...
    start_y = (road.GetStart().y < road.GetEnd().y) ? (road.GetStart().y)
                                                    : (road.GetEnd().y);
    end_y = (road.GetStart().y < road.GetEnd().y) ? (road.GetEnd().y)
                                                  : (road.GetStart().y);
  ...
  return ((position.x > start_x) ||
          (std::abs(position.x - start_x) < EPSILON)) &&
