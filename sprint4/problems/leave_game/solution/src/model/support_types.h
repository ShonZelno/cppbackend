...
struct VelocityHasher {
  size_t operator()(const Velocity &velocity) const {
    size_t sd = 17;
    return std::hash<double>{}(velocity.vy) * sd +
           std::hash<double>{}(velocity.vx);
  }
};

const std::unordered_map<Velocity, Direction, VelocityHasher>
    VELOCITY_TO_DIRECTION = {{{0, -1}, Direction::NORTH},
                             {{0, 1}, Direction::SOUTH},
                             {{-1, 0}, Direction::WEST},
                             {{1, 0}, Direction::EAST},
                             {{0, 0}, Direction::NONE}};
