...
FindGatherEvents(const ItemGathererProvider &provider) {
  ...
  for (size_t g = 0; g < provider.GatherersCount(); ++g) {
    for (size_t i = 0; i < provider.ItemsCount(); ++i) {
      ...
      if ((!(gatherer.end_pos.x != gatherer.start_pos.x ||
             gatherer.end_pos.y != gatherer.start_pos.y))) {
        continue;
      }
      auto collect_result = TryCollectPoint(
          gatherer.start_pos, gatherer.end_pos, item.GetPosition());
      if (collect_result.IsCollected(item.GetWidth() + gatherer.width)) {
        result.push_back(
            {i, g, collect_result.sq_distance, collect_result.proj_ratio});
      }
    }
  }
