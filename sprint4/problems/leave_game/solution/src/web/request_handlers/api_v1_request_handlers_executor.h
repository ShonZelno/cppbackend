...
template <typename Request, typename Send> class ApiV1RequestHandlerExecutor {
  using ActivatorType = bool (*)(const Request &);
  using HandlerType = std::optional<size_t> (*)(
  ...
  bool Execute(const Request &req,
    ...
    for (auto item : rh_storage_) {
      if (item.GetActivator()(req)) {
        auto res = item.GetHandler(req.method())(req, application,
                                                 std::forward<Send>(send));
        while (res.has_value()) {
          res = item.GetEmergeHandlerByIndex(res.value())(
              req, application, std::forward<Send>(send));
        }
        return true;
      }
    }
...
private:
  std::vector<RequestHandlerNode<ActivatorType, HandlerType>> rh_storage_ = {
      ...
      RequestHandlerNode<ActivatorType, HandlerType>(
          ...
          InvalidMethodHandler, {MapNotFoundHandler}),

      RequestHandlerNode<ActivatorType, HandlerType>(
          InvalidContentTypeActivator,
          {{http::verb::post, InvalidContentTypeHandler}},
          InvalidContentTypeHandler),

      RequestHandlerNode<ActivatorType, HandlerType>(
          JoinToGameInvalidJsonReqActivator,
          {{http::verb::post, JoinToGameInvalidJsonReqHandler}},
          OnlyPostMethodAllowedHandler),
      RequestHandlerNode<ActivatorType, HandlerType>(
          JoinToGameEmptyPlayerNameActivator,
          {{http::verb::post, JoinToGameEmptyPlayerNameHandler}},
          OnlyPostMethodAllowedHandler),
      RequestHandlerNode<ActivatorType, HandlerType>(
          JoinToGameActivator, {{http::verb::post, JoinToGameHandler}},
          OnlyPostMethodAllowedHandler, {JoinToGameMapNotFoundHandler}),
      ...
      RequestHandlerNode<ActivatorType, HandlerType>(
          GetPlayersListActivator,
          {{http::verb::get, GetPlayersListHandler},
           {http::verb::head, GetPlayersListHandler}},
          InvalidMethodHandler, {UnknownTokenHandler}),
      RequestHandlerNode<ActivatorType, HandlerType>(
          GetGameStateActivator,
          {{http::verb::get, GetGameStateHandler},
           {http::verb::head, GetGameStateHandler}},
          InvalidMethodHandler, {UnknownTokenHandler}),

      RequestHandlerNode<ActivatorType, HandlerType>(
          PlayerActionInvalidActionActivator,
          {{http::verb::post, PlayerActionInvalidActionHandler}},
          OnlyPostMethodAllowedHandler),
      RequestHandlerNode<ActivatorType, HandlerType>(
          PlayerActionActivator, {{http::verb::post, PlayerActionHandler}},
          InvalidMethodHandler, {UnknownTokenHandler}),

      RequestHandlerNode<ActivatorType, HandlerType>(
          TimeTickInvalidMsgActivator,
          {{http::verb::post, TimeTickInvalidMsgHandler}}, InvalidMethodHandler,
          {InvalidEndpointHandler}),
      RequestHandlerNode<ActivatorType, HandlerType>(
          TimeTickActivator, {{http::verb::post, TimeTickHandler}},
          InvalidMethodHandler, {InvalidEndpointHandler}),
      RequestHandlerNode<ActivatorType, HandlerType>(
          GetRecordsActivator,
