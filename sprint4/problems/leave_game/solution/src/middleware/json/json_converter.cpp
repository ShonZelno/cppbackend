...
std::string CreatePlayersListOnMapResponse(
  ...
  for (auto player : players) {
    ...
    ss << *(player->GetId());
    json::value jv_item = {
        {json_keys::RESPONSE_PLAYER_NAME, player->GetName()}};
    obj[ss.str()] = jv_item;
  }
...
std::string CreateGameStateResponse(
  ...
  for (auto player : players) {
    ...
    json::value jv_item = {{json_keys::RESPONSE_DOG_POSITION, pos},
                           ...
                           {json_keys::RESPONSE_DOG_DIRECTION,
                            model::DIRECTION_TO_STRING.at(dog->GetDirection())},
  ...
  for (auto [id, lost_object] : lost_objects) {
    std::stringstream ss;
    ss << *id;
    json::value loot = {{json_keys::LOST_OBJECTS_TYPE,
                         json::value_from(lost_object->GetType())},
                        {json_keys::LOST_OBJECTS_POSITION,
                         json::array{lost_object->GetPosition().x,
                                     lost_object->GetPosition().y}}};
    loots[ss.str()] = loot;
  }
...
ParseJoinToGameRequest(const std::string &msg) {
  try {
    ...
    std::string player_name = json::value_to<std::string>(
        jv.as_object().at(json_keys::REQUEST_PLAYER_NAME));
    model::Map::Id map_id{json::value_to<std::string>(
        jv.as_object().at(json_keys::REQUEST_MAP_ID))};
...
std::optional<std::string> ParsePlayerActionRequest(const std::string &msg) {
  try {
    json::value jv = json::parse(msg);
    std::string direction = json::value_to<std::string>(
        jv.as_object().at(json_keys::REQUEST_PLAYER_MOVE));
    return direction;
  } catch (...) {
    return std::nullopt;
  }
...
std::optional<int> ParseSetDeltaTimeRequest(const std::string &msg) {
  try {
    ...
    int time_delta =
        json::value_to<int>(jv.as_object().at(json_keys::REQUEST_TIME_DELTA));
    return time_delta;
  } catch (...) {
    return std::nullopt;
  }
