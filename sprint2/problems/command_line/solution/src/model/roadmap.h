...
namespace model
{
    ...
    private:
        ...
        RectBounds CalculateRoadBounds(const Road& road, double offset);
        bool IsValidPositionOnRoad(const Road &road, const Position &position);
        void CopyContent(const Roads &roads);
        void RoadScalling(Coord start, Coord end, Coord start_other);

        template <bool IsXAxis>
        std::pair<int64_t, int64_t> CalculateMovement(const Velocity &old_velocity,
